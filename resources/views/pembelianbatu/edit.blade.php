...
<title>Pembelian Batu</title>


<body>
    <h1 class="text-center mb-4">Edit Pembelian Batu</h1>
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-8">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('pembelianbatu.update', $item->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_customer" class="form-control">
                                    ...
                                    <option value="{{ $customer->id }}">{{ $customer->namacust }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="id_batu" class="form-control">
                                    @foreach ($masterbatu as $batu)
                                    <option value="{{ $batu->id }}">{{ $batu->jenisbatu }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputPassword1">Keperluan</label>
                                <input value="{{ $item->berapaton }}" type="number" name="berapaton"
